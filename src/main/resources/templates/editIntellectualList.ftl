...
<head>
    <meta charset="UTF-8">
    <title>Интеллектуальные собственности</title>
...
<!-- NAVBAR -->
<h2 align="center" style="margin-top: 20px">Интеллектуальные собственности</h2>
<form method="post">
    <div class="form-style-2">
        <label for="year">Поиск по названию
    ...
    <table style="margin-top: 50px" align="center" class="table">
        <tr>
            ...
            <th class="th">Объект</th>
            <th class="th">Правообладатель</th>
            <th class="th">Дата публикаци</th>
        </tr>
        <#list propList as prop>
            <tr>
                <td class="td">${prop.getName()}</td>
                <td class="td">${prop.getObject()}</td>
                <td class="td">${prop.getOwner()}</td>
                <td class="td">${prop.getDateOfPublicationShow()}</td>
                <td class="td"><a href="/progPage/${prop.getId()}">Информация</a></td>
            </tr>
        </#list>
