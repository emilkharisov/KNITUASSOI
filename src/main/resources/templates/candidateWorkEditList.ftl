...
<!-- NAVBAR -->
<h2 align="center" style="margin-top: 20px">Кандидатские | Докторские работы</h2>
<form method="post">
    <div class="form-style-2">
        <label for="name">Поиск по названию
    ...
    <table style="margin-top: 50px" align="center" class="table">
        <tr>
            ...
            <th class="th">Тип</th>
            <th class="th">Дата защиты</th>
            <th class="th">Автор</th>
        </tr>
        <#list candidateWorks as candidateWork>
            <tr>
                <td class="td">${candidateWork.getName()}</td>
                <td class="td">${candidateWork.getType()}</td>
                <td class="td">${candidateWork.getDateShow()}</td>

                <#assign workerName="${candidateWork.getWorker().getLastname()}" />
                <#assign workerR="${candidateWork.getWorker().getPosition()}" />

                <td class="td">${workerName} ${workerR}</td>
                <td class="td"><a href="/candidatePage/${candidateWork.getId()}">Информация</a></td>
            </tr>
        </#list>
    </table>
</form>
