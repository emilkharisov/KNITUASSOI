...
<div class="form-style-2">
    <div class="form-style-2-heading"><font color="white"></font>
        Выборка конференций по году
    </div>
    <form method="post" action="/getСonfExctract">
        ...
        <#list map as key, value>
            <br>
          <h2>${key.getName()} ${key.getDateOfStartShow()} ${key.getCity()} ${key.getLevel()}</h2>
            <table style="margin-top: 50px" align="center" class="table">
                ...
                <#list value as val>
                    <tr>
                        ...
                        <td class="td">${val.getDateOfBirthShow()}</td>
                        <td class="td">${val.getPosition()}</td>
                    </tr>
                </#list>
            </table>
        </#list>
