...
<table cellpadding="20">
    <tr>
        <td>
            <img src="/static/img/candidateWork.png" class="round" width="300px" height="300px">
            <table style="margin-left: 70px">
                <tr>
                    <td><button onclick="location.href='http://localhost:8080/candidateDelete/${candidateWork.getId()}'"><img src="/static/img/trashButton.png" height="60px" width="60px"></button></td>
                    <td><button onclick="location.href='http://localhost:8080/candidateChange/${candidateWork.getId()}'"><img src="/static/img/changeButton.png" height="60px" width="60px"></button></td>
                </tr>
            </table>
        </td>
        <td>
            <h2 style="margin-left: 20px">${candidateWork.getName()}</h2>
            <table cellpadding="20" class="classTable">
                <tr>
                    <td><label>Тип</label></td>
                    <td>${candidateWork.getType()}</td>
                </tr>

                <#assign workerName="${candidateWork.getWorker().getFirstname()}" />
                <#assign workerLatname="${candidateWork.getWorker().getLastname()}" />
                <#assign workerR="${candidateWork.getWorker().getPosition()}" />
                <tr>
                    <td><label>Автор</label></td>
                    <td> ${workerLatname} ${workerName} ${workerR}</td>
                </tr>
