...
<div class="form-style-2">
    <div class="form-style-2-heading"><font color="white"></font>
        Выборка преподавателей, у которых не было повышения квалификации за n лет
    </div>
    <form method="post" action="/getQualificationAfter">
        <label for="year">Введите количество лет
            <input class="input-field" id="year" name="year">
        </label>
        <br>
        <h4>По умолчанию = 3 года</h4>
        <br>
        ...
        <table style="margin-top: 50px" align="center" class="table">
            <tr>
                ...
                <th class="th">Дата квалификации</th>
            </tr>
            <#list workers as worker>
                <tr>
                    ...
                    <#if worker.getQualificationDate()??>
                        <td class="td">${worker.getQualificationDate()}</td>
                        <#else>
                        <td class="td">-</td>
                    </#if>
                </tr>
